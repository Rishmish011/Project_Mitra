...
class MANEpidemicApp(tk.Tk):
    ...
    def _create_tooltip(self, widget, text):
        ...
        tooltip = tk.Label(widget, text=text, background="#FFFFEA", 
    ...
    def _save_dashboard_figure(self):
        """Save dashboard figures to file with enhanced UX."""
        # Determine which content to save
        figure_type = self.figure_selection.get()
        
        # Create appropriate figure based on selection
        if figure_type == "Complete Dashboard":
            fig = self.fig  # Use the existing combined figure
            default_name = "epidemic_dashboard"
        else:
            # Create a new standalone figure with better dimensions for single plot
            fig = Figure(figsize=(10, 6))
            ax = fig.add_subplot(111)
            
            if "Epidemic Curves" in figure_type:
                # Copy data from ax1 to new figure
                ...
                ax.legend()
                default_name = "epidemic_curves"
            elif "R Effective" in figure_type:
                # Copy data from ax2 to new figure
                ...
                default_name = "r_effective"
            else:  # Policy Measures
                # Copy data from ax3 to new figure
                ...
                ax.set_ylabel("Level")
                ax.legend()
                default_name = "policy_measures"
                
            ax.grid(True)
            fig.tight_layout()
        
        # Get format from selection
        format_type = self.format_selection.get()
        if "PNG" in format_type:
            ext = ".png"
        elif "JPEG" in format_type:
            ext = ".jpg"
        ...
        if filename:
            try:
                ...
                # If we created a new figure, close it to free memory
                if fig != self.fig:
                    plt.close(fig)
                    
                # Reset cursor and show success message
                self.config(cursor="")
                self._update_status(f"Figure saved to {filename}")
                
                # Ask if user wants to open the saved file
                if ext in [".png", ".jpg"]:
                    open_result = messagebox.askyesno(
                        "Success", 
                        f"Figure saved successfully to:\n{filename}\n\nWould you like to open it now?"
                    )
                    if open_result:
    ...
    def _create_menu(self):
        ...
        file_menu.add_command(label="Save Figure...", command=self._save_dashboard_figure)
        file_menu.add_separator()
        ...
        analysis_menu.add_command(label="Compare Diffusion Modes", command=self._run_diffusion_comparison)
        menubar.add_cascade(label="Analysis", menu=analysis_menu)
        
        # Help menu
        help_menu = tk.Menu(menubar, tearoff=0)
        help_menu.add_command(label="User Guide", command=self._show_user_guide)
        help_menu.add_command(label="Information Diffusion Modes", command=self._show_diffusion_modes_help)
    ...
    def _create_dashboard_tab(self):
        ...
        # Add information mode indicator
        self.info_mode_indicator_var = tk.StringVar(value="Info Mode: Uniform")
        info_mode_label = ttk.Label(status_frame, textvariable=self.info_mode_indicator_var)
        info_mode_label.pack(anchor=tk.W, pady=2)
        
        # Create tooltip for info mode
        self._create_tooltip(info_mode_label, 
                          "Current information diffusion model:\n"
                          "Uniform: Simple homogeneous spread\n"
                          "Gossip: Network-based, follows realistic social spreading patterns")
        ...
        # Add save control panel
        save_frame = ttk.Frame(display_frame)
        save_frame.pack(fill=tk.X, pady=5)
        
        # Left side: Save button with icon
        save_btn = ttk.Button(save_frame, text="Save Figure", command=self._save_dashboard_figure)
        save_btn.pack(side=tk.LEFT, padx=5)
        
        # Middle: Figure selector
        ttk.Label(save_frame, text="Content:").pack(side=tk.LEFT, padx=(10, 2))
        self.figure_selection = tk.StringVar(value="Complete Dashboard")
        figure_combo = ttk.Combobox(save_frame, textvariable=self.figure_selection, width=20, state="readonly")
        figure_combo.pack(side=tk.LEFT, padx=2)
        figure_combo['values'] = ["Complete Dashboard", "Epidemic Curves Only", 
                                 "R Effective Only", "Policy Measures Only"]
        
        # Right side: Format selector
        ttk.Label(save_frame, text="Format:").pack(side=tk.LEFT, padx=(10, 2))
        self.format_selection = tk.StringVar(value="PNG (.png)")
        format_combo = ttk.Combobox(save_frame, textvariable=self.format_selection, width=12, state="readonly")
        format_combo.pack(side=tk.LEFT, padx=2)
        format_combo['values'] = ["PNG (.png)", "JPEG (.jpg)", "PDF (.pdf)", "SVG (.svg)"]
        
        help_btn = ttk.Button(save_frame, text="?", width=2, cursor="question_arrow")
        help_btn.pack(side=tk.LEFT, padx=5)
        
        # Create tooltip for help
        self._create_tooltip(help_btn, "Save the current figure in various formats.\n"
                            "• Complete Dashboard: Saves all three plots\n"
                            "• Individual options: Save just one plot\n"
                            "Choose your preferred file format from the dropdown.")
    ...
    def _create_parameters_tab(self):
        ...
        # Add Information Diffusion section
        info_diff_frame = ttk.LabelFrame(scrollable_frame, text="Information Diffusion")
        info_diff_frame.pack(fill=tk.X, padx=10, pady=5, ipadx=5, ipady=5)
        
        # Information diffusion mode
        mode_frame = ttk.Frame(info_diff_frame)
        mode_frame.pack(fill=tk.X, pady=2)
        ttk.Label(mode_frame, text="Diffusion Mode:").pack(side=tk.LEFT)
        self.info_mode_var = tk.StringVar(value=self.params.information_diffusion_mode)
        mode_combo = ttk.Combobox(mode_frame, textvariable=self.info_mode_var, width=15)
        mode_combo['values'] = ["uniform", "gossip"]
        mode_combo.pack(side=tk.RIGHT)
        
        # Add help button for diffusion modes
        help_btn = ttk.Button(mode_frame, text="?", width=2, command=self._show_diffusion_modes_help)
        help_btn.pack(side=tk.RIGHT, padx=5)
        
        # Add tooltip to explain modes
        self._create_tooltip(mode_combo, 
                           "uniform: Traditional homogeneous information spread\n"
                           "gossip: Network-based information diffusion using gossip dynamics")
        
        # Gossip transmission probability (only used in gossip mode)
        tp_frame = ttk.Frame(info_diff_frame)
        tp_frame.pack(fill=tk.X, pady=2)
        ttk.Label(tp_frame, text="Gossip Transmission Probability:").pack(side=tk.LEFT)
        self.gossip_prob_var = tk.DoubleVar(value=self.params.gossip_transmission_probability)
        ttk.Spinbox(tp_frame, from_=0.0, to=1.0, increment=0.05, 
                  textvariable=self.gossip_prob_var, width=10).pack(side=tk.RIGHT)
        
        # Information hop limit (only used in gossip mode)
        hop_frame = ttk.Frame(info_diff_frame)
        hop_frame.pack(fill=tk.X, pady=2)
        ttk.Label(hop_frame, text="Information Hop Limit:").pack(side=tk.LEFT)
        self.hop_limit_var = tk.IntVar(value=self.params.information_hop_limit)
        ttk.Spinbox(hop_frame, from_=1, to=5, increment=1, 
                  textvariable=self.hop_limit_var, width=10).pack(side=tk.RIGHT)
    ...
    def _create_behavior_tab(self):
        ...
        compare_frame = ttk.LabelFrame(control_frame, text="Compare Diffusion Modes")
        compare_frame.pack(fill=tk.X, pady=5)
        
        self.compare_diffusion_var = tk.BooleanVar(value=False)
        ttk.Checkbutton(compare_frame, text="Show Comparison", 
                       variable=self.compare_diffusion_var,
                       command=self._update_behavior_viz).pack(anchor=tk.W, pady=2)
        
        ttk.Button(compare_frame, text="Run Comparison", 
                  command=self._run_diffusion_comparison).pack(fill=tk.X, pady=5)
    ...
    def _create_gossip_tab(self):
        """Create the gossip dynamics visualization tab."""
        gossip_tab = ttk.Frame(self.notebook)
        self.notebook.add(gossip_tab, text="Gossip Dynamics")
        
        # Split into control panel and visualization
        control_frame = ttk.Frame(gossip_tab, width=300)
        control_frame.pack(side=tk.LEFT, fill=tk.Y, padx=5, pady=5)
        
        viz_frame = ttk.Frame(gossip_tab)
        viz_frame.pack(side=tk.RIGHT, fill=tk.BOTH, expand=True, padx=5, pady=5)
        
        # Gossip parameter controls
        param_frame = ttk.LabelFrame(control_frame, text="Gossip Parameters")
        param_frame.pack(fill=tk.X, pady=5)
        
        # Transmission probability
        tp_frame = ttk.Frame(param_frame)
        tp_frame.pack(fill=tk.X, pady=2)
        ttk.Label(tp_frame, text="Transmission Probability:").pack(side=tk.LEFT)
        self.gossip_trans_prob_var = tk.DoubleVar(value=self.params.gossip_transmission_probability)
        tp_spinner = ttk.Spinbox(tp_frame, from_=0.0, to=1.0, increment=0.05, 
                               textvariable=self.gossip_trans_prob_var, width=10)
        tp_spinner.pack(side=tk.RIGHT)
        
        # Hop limit
        hl_frame = ttk.Frame(param_frame)
        hl_frame.pack(fill=tk.X, pady=2)
        ttk.Label(hl_frame, text="Information Hop Limit:").pack(side=tk.LEFT)
        self.hop_limit_var = tk.IntVar(value=self.params.information_hop_limit)
        hl_spinner = ttk.Spinbox(hl_frame, from_=1, to=5, increment=1, 
                               textvariable=self.hop_limit_var, width=10)
        hl_spinner.pack(side=tk.RIGHT)
        
        # Apply button
        ttk.Button(param_frame, text="Apply Parameters", 
                  command=self._apply_gossip_parameters).pack(fill=tk.X, pady=5)
        ...
        # What to visualize
        self.gossip_viz_var = tk.StringVar(value="Spread Factor")
        ttk.Radiobutton(viz_options, text="Spread Factor", variable=self.gossip_viz_var, 
                       value="Spread Factor", command=self._update_gossip_viz).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(viz_options, text="Spreading Time", variable=self.gossip_viz_var, 
                       value="Spreading Time", command=self._update_gossip_viz).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(viz_options, text="Gossip Vulnerability", variable=self.gossip_viz_var, 
                       value="Vulnerability", command=self._update_gossip_viz).pack(anchor=tk.W, pady=2)
        ttk.Radiobutton(viz_options, text="Time Series", variable=self.gossip_viz_var, 
                       value="Time Series", command=self._update_gossip_viz).pack(anchor=tk.W, pady=2)
        
        # Network statistics
        stats_frame = ttk.LabelFrame(control_frame, text="Gossip Statistics")
        stats_frame.pack(fill=tk.X, pady=5)
        
        self.gossip_stats_text = ScrolledText(stats_frame, height=10, width=30)
        self.gossip_stats_text.pack(fill=tk.BOTH, expand=True, pady=5)
        
        # Create matplotlib figure for gossip visualization
        self.gossip_fig = Figure(figsize=(8, 6))
        self.gossip_ax = self.gossip_fig.add_subplot(111)
        
        self.gossip_canvas = FigureCanvasTkAgg(self.gossip_fig, master=viz_frame)
        self.gossip_canvas.draw()
        self.gossip_canvas.get_tk_widget().pack(fill=tk.BOTH, expand=True)
        
        # Add toolbar
        gossip_toolbar_frame = ttk.Frame(viz_frame)
        gossip_toolbar_frame.pack(fill=tk.X)
        gossip_toolbar = NavigationToolbar2Tk(self.gossip_canvas, gossip_toolbar_frame)
        gossip_toolbar.update()
    ...
    def _apply_gossip_parameters(self):
        """Apply gossip parameter changes to the model."""
        self.params.gossip_transmission_probability = self.gossip_trans_prob_var.get()
        self.params.information_hop_limit = self.hop_limit_var.get()
        
        if hasattr(self.model, 'gossip_model'):
            # Force update of all gossip metrics
            self.model.gossip_model.update_all_metrics()
            
        self._update_gossip_viz()
        self._update_status("Applied gossip parameters")
    
    def _update_gossip_viz(self, *args):
        """Update the gossip visualization based on current selection."""
        if not hasattr(self.model, 'gossip_model'):
            # If gossip model not available, show message
            self.gossip_ax.clear()
            self.gossip_ax.text(0.5, 0.5, "Gossip model not active.\nEnable gossip diffusion mode in Parameters tab.",
                             horizontalalignment='center', verticalalignment='center',
                             transform=self.gossip_ax.transAxes)
            self.gossip_canvas.draw()
            return
        ...
        viz_type = self.gossip_viz_var.get()
        
        if viz_type == "Spread Factor":
            # Plot spread factor vs degree
            degrees = []
            spread_factors = []
            
            for ind in self.model.population:
                degree = len(list(self.model.contact_network.networks[NetworkScale.MICRO].neighbors(ind.idx)))
                sf = self.model.gossip_model.spread_factors.get(ind.idx, 0)
                
                degrees.append(degree)
                spread_factors.append(sf)
            
            self.gossip_ax.scatter(degrees, spread_factors, alpha=0.5)
            self.gossip_ax.set_xlabel('Degree (k)')
            self.gossip_ax.set_ylabel('Spread Factor (f)')
            self.gossip_ax.set_title('Spread Factor vs. Degree')
            self.gossip_ax.grid(True)
            
            # Calculate optimal degree
            k0 = self.model.gossip_model.get_optimal_degree()
            self.gossip_ax.axvline(x=k0, color='r', linestyle='--', label=f'Optimal k₀ = {k0:.1f}')
            self.gossip_ax.legend()
            
        elif viz_type == "Spreading Time":
            # Plot spreading time vs degree
            degrees = []
            times = []
            
            for ind in self.model.population:
                degree = len(list(self.model.contact_network.networks[NetworkScale.MICRO].neighbors(ind.idx)))
                st = self.model.gossip_model.spreading_times.get(ind.idx, 0)
                
                degrees.append(degree)
                times.append(st)
            
            self.gossip_ax.scatter(degrees, times, alpha=0.5)
            self.gossip_ax.set_xlabel('Degree (k)')
            self.gossip_ax.set_ylabel('Spreading Time (τ)')
            self.gossip_ax.set_title('Spreading Time vs. Degree')
            self.gossip_ax.grid(True)
            
            # Fit logarithmic curve to data following paper's finding
            if degrees and times:
                # Filter out zeros
                valid_points = [(d, t) for d, t in zip(degrees, times) if d > 0 and t > 0]
                if valid_points:
                    d_valid, t_valid = zip(*valid_points)
                    
                    # Fit logarithmic function τ = A + B ln(k)
                    def log_func(x, A, B):
                        return A + B * np.log(x)
                        
                    try:
                        from scipy import optimize
                        params, _ = optimize.curve_fit(log_func, d_valid, t_valid)
                        
                        # Plot fitted curve
                        x_fit = np.linspace(min(d_valid), max(d_valid), 100)
                        y_fit = log_func(x_fit, *params)
                        self.gossip_ax.plot(x_fit, y_fit, 'r-', 
                                          label=f'τ = {params[0]:.2f} + {params[1]:.2f} ln(k)')
                        self.gossip_ax.legend()
                    except:
                        pass  # If curve fitting fails
            
        elif viz_type == "Vulnerability":
            # Plot vulnerability vs degree
            degrees = []
            vulnerabilities = []
            
            for ind in self.model.population:
                degree = len(list(self.model.contact_network.networks[NetworkScale.MICRO].neighbors(ind.idx)))
                vuln = self.model.gossip_model.calculate_vulnerability(ind.idx)
                
                degrees.append(degree)
                vulnerabilities.append(vuln)
            
            self.gossip_ax.scatter(degrees, vulnerabilities, alpha=0.5)
            self.gossip_ax.set_xlabel('Degree (k)')
            self.gossip_ax.set_ylabel('Gossip Vulnerability')
            self.gossip_ax.set_title('Gossip Vulnerability vs. Degree')
            self.gossip_ax.grid(True)
            
            # Plot the theoretical vulnerability curve
            k0 = self.model.gossip_model.get_optimal_degree()
            x = np.linspace(1, max(degrees) if degrees else 30, 100)
            y = [1.0 - self.params.optimal_degree_effect * np.exp(-0.2 * ((k - k0)/k0)**2) for k in x]
            
            self.gossip_ax.plot(x, y, 'r-', label=f'Theoretical (k₀ = {k0:.1f})')
            self.gossip_ax.legend()
            
        else:  # Time Series
            # Plot time series of average spread factor and spreading time
            if hasattr(self.model, 'spread_factor_history') and self.model.spread_factor_history:
                times, sf_values = zip(*self.model.spread_factor_history)
                self.gossip_ax.plot(times, sf_values, 'b-', label='Avg. Spread Factor')
                
            if hasattr(self.model, 'spreading_time_history') and self.model.spreading_time_history:
                times, st_values = zip(*self.model.spreading_time_history)
                self.gossip_ax.plot(times, st_values, 'r-', label='Avg. Spreading Time')
                
            self.gossip_ax.set_xlabel('Time (days)')
            self.gossip_ax.set_title('Gossip Dynamics Over Time')
            self.gossip_ax.grid(True)
            self.gossip_ax.legend()
        
        # Update statistics
        self._update_gossip_stats()
        
        # Draw the plot
        self.gossip_canvas.draw()
        
    def _update_gossip_stats(self):
        """Update the gossip statistics display."""
        if not hasattr(self.model, 'gossip_model'):
            self.gossip_stats_text.delete(1.0, tk.END)
            self.gossip_stats_text.insert(tk.END, "Gossip model not active.\nEnable gossip diffusion mode in Parameters tab.")
            return
            
        self.gossip_stats_text.delete(1.0, tk.END)
        
        # Calculate statistics
        spread_factors = list(self.model.gossip_model.spread_factors.values())
        spreading_times = list(self.model.gossip_model.spreading_times.values())
        
        if spread_factors:
            self.gossip_stats_text.insert(tk.END, f"Average Spread Factor: {np.mean(spread_factors):.4f}\n")
            self.gossip_stats_text.insert(tk.END, f"Max Spread Factor: {max(spread_factors):.4f}\n")
            self.gossip_stats_text.insert(tk.END, f"Min Spread Factor: {min(spread_factors):.4f}\n")
            
        if spreading_times:
            self.gossip_stats_text.insert(tk.END, f"Average Spreading Time: {np.mean(spreading_times):.4f}\n")
            self.gossip_stats_text.insert(tk.END, f"Max Spreading Time: {max(spreading_times):.4f}\n")
            self.gossip_stats_text.insert(tk.END, f"Min Spreading Time: {min(spreading_times):.4f}\n")
            
        # Calculate optimal degree
        k0 = self.model.gossip_model.get_optimal_degree()
        self.gossip_stats_text.insert(tk.END, f"Optimal Degree (k₀): {k0:.2f}\n")
        
        # Calculate fraction of nodes with degree near optimal
        if hasattr(self.model, 'population'):
            near_optimal = 0
            for ind in self.model.population:
                degree = len(list(self.model.contact_network.networks[NetworkScale.MICRO].neighbors(ind.idx)))
                if abs(degree - k0) / k0 < 0.2:  # Within 20% of optimal
                    near_optimal += 1
                    
            fraction = near_optimal / len(self.model.population)
            self.gossip_stats_text.insert(tk.END, f"Fraction near optimal: {fraction:.2f}\n")
    
    def _run_diffusion_comparison(self):
        """Run simulations with both diffusion modes for comparison."""
        if not self.model.initialized:
            messagebox.showerror("Error", "Please initialize the model first")
            return
        
        # Store current settings
        current_mode = self.params.information_diffusion_mode
        ...
        # Run with uniform diffusion
        self.params.information_diffusion_mode = "uniform"
        self.model.params = self.params
        self.model.behavior_model.gossip_model = None
        
        # Run simulation
        self._run_simulation_for_comparison(uniform_results)
        
        # Run with gossip diffusion
        self.params.information_diffusion_mode = "gossip"
        self.model.params = self.params
        self.model.gossip_model = GossipPropagation(self.model, self.params)
        self.model.behavior_model.gossip_model = self.model.gossip_model
        
        # Run simulation
        self._run_simulation_for_comparison(gossip_results)
        
        # Restore original settings
        self.params.information_diffusion_mode = current_mode
        self.model = MANEpidemicModel(self.params)  # Reset to clean state
        
        # Plot comparison
        self._plot_diffusion_comparison(uniform_results, gossip_results)
        
        # Update status
        self._update_status("Diffusion mode comparison completed")
        
    def _run_simulation_for_comparison(self, results_dict):
        """Run a simulation and store results for comparison."""
        self.model.initialize()
        self.model.simulate(self.days_var.get())
        
        # Store relevant results
        results_dict["info_history"] = self.model.behavior_model.info_history.copy()
        results_dict["case_counts"] = {
            state: data.copy() for state, data in self.model.case_counts.items()
        }
        results_dict["R_effective"] = self.model.R_effective.copy()
        
    def _plot_diffusion_comparison(self, uniform_results, gossip_results):
        """Plot comparison between diffusion modes."""
        # Create comparison window
        comparison_window = tk.Toplevel(self)
        comparison_window.title("Diffusion Mode Comparison")
        ...
        unif_times, unif_info = zip(*uniform_results["info_history"])
        gossip_times, gossip_info = zip(*gossip_results["info_history"])
        
        ax1.plot(unif_times, unif_info, 'b-', label='Uniform')
        ax1.plot(gossip_times, gossip_info, 'r-', label='Gossip')
        ax1.set_title("Information Level Comparison")
        ax1.set_ylabel("Information Level")
        ax1.legend()
        
        # Case comparison
        ax2 = fig.add_subplot(312)
        for results, label, color in [(uniform_results, 'Uniform', 'b'), 
                                     (gossip_results, 'Gossip', 'r')]:
            times, counts = zip(*results["case_counts"][HealthState.INFECTIOUS])
            ax2.plot(times, counts, color=color, label=f'{label}')
        
        ax2.set_title("Infectious Cases Comparison")
        ax2.set_ylabel("Number of Cases")
        ax2.legend()
        
        # R effective comparison
        ax3 = fig.add_subplot(313)
        unif_r_times, unif_r_values = zip(*uniform_results["R_effective"])
        gossip_r_times, gossip_r_values = zip(*gossip_results["R_effective"])
        
        ax3.plot(unif_r_times, unif_r_values, 'b-', label='Uniform')
        ax3.plot(gossip_r_times, gossip_r_values, 'r-', label='Gossip')
        ax3.set_title("R Effective Comparison")
        ...
        # Add toolbar
        toolbar_frame = ttk.Frame(comparison_window)
        toolbar_frame.pack(fill=tk.X)
        toolbar = NavigationToolbar2Tk(canvas, toolbar_frame)
        toolbar.update()
        
    def _show_diffusion_modes_help(self):
        """Show help information about diffusion modes."""
        help_text = """
        Information Diffusion Modes
        ==========================
        
        This model offers two ways that information spreads during an epidemic:
        
        1. Uniform Diffusion (Traditional)
        ---------------------------------
        In this mode, information about the epidemic spreads evenly throughout the 
        population, regardless of network structure. This is the classic approach 
        used in most epidemic models, where information is directly proportional 
        to case counts.
        
        Key characteristics:
        - Information spreads homogeneously
        - All individuals have equal access to information
        ...
        Key characteristics:
        - Information spreads through network connections
        - Network position affects information access
        - Introduces spread factor and spreading time metrics
        - Shows optimal degree phenomenon (some network positions are less vulnerable)
        - More computationally intensive but more realistic
        - Can create information inequities in population
        
        When to use each mode:
        - Use Uniform for simple scenarios and faster computation
        - Use Gossip for more realistic simulations and to study how network 
          structure affects epidemic outcomes through information spread
        
        The Gossip Dynamics tab provides detailed analysis when gossip-based 
        diffusion is enabled.
        """
        
        help_window = tk.Toplevel(self)
        help_window.title("Information Diffusion Modes Help")
    ...
    def _initialize_model(self):
        ...
        try:
            ...
            # Update info mode display
            self.info_mode_indicator_var.set(f"Info Mode: {self.params.information_diffusion_mode.capitalize()}")
        ...
        if filename:
            try:
                ...
                with open(filename, 'w') as f:
                    ...
                    f.write(f"<p>Information Diffusion Mode: {self.params.information_diffusion_mode.capitalize()}</p>\n")
                    ...
                    # Add gossip stats if available
                    if self.params.information_diffusion_mode == "gossip" and hasattr(self.model, 'gossip_model'):
                        f.write("<h2>Gossip Dynamics Statistics</h2>\n")
                        f.write("<table>\n")
                        f.write("<tr><th>Metric</th><th>Value</th></tr>\n")
                        ...
                        if spread_factors:
                            f.write(f"<tr><td>Average Spread Factor</td><td>{np.mean(spread_factors):.4f}</td></tr>\n")
                        
                        if spreading_times:
                            f.write(f"<tr><td>Average Spreading Time</td><td>{np.mean(spreading_times):.4f}</td></tr>\n")
                            
                        # Calculate optimal degree
                        k0 = self.model.gossip_model.get_optimal_degree()
                        f.write(f"<tr><td>Optimal Degree (k₀)</td><td>{k0:.2f}</td></tr>\n")
    ...
    def _show_user_guide(self):
        ...
        3. Analyzing Results
           ...
           - The Gossip tab explores information diffusion dynamics
           - The Data tab allows importing and assimilating real-world data
        
        4. Information Diffusion Modes
           - Choose between uniform (traditional) or gossip-based (network-aware) 
             information diffusion in the Parameters tab
           - Compare results between modes using the Analysis > Compare Diffusion Modes menu
    ...
    def _show_about(self):
        ...
        - Network-aware information diffusion with gossip dynamics
    ...
    def _apply_parameters(self):
        ...
        new_params.information_diffusion_mode = self.info_mode_var.get()
        new_params.gossip_transmission_probability = self.gossip_prob_var.get()
        new_params.information_hop_limit = self.hop_limit_var.get()
        ...
        if not hasattr(self.model, 'initialized') or not self.model.initialized:
            ...
            # If model is already initialized and diffusion mode changed,
            # we need to reinitialize or update accordingly
            old_mode = self.model.params.information_diffusion_mode
            new_mode = new_params.information_diffusion_mode
            self.model.params = self.params
            
            if old_mode != new_mode:
                if new_mode == "gossip" and not hasattr(self.model, 'gossip_model'):
                    # Initialize gossip model if switching to gossip mode
                    self.model.gossip_model = GossipPropagation(self.model, self.params)
                    self.model.behavior_model.gossip_model = self.model.gossip_model
                    self.model.spread_factor_history = []
                    self.model.spreading_time_history = []
                    self.model.gossip_model.update_all_metrics()
                    self._update_status("Switched to gossip-based information diffusion")
                    
                    # Enable gossip tab
                    self.notebook.tab(4, state="normal")
                elif new_mode == "uniform" and hasattr(self.model, 'gossip_model'):
                    # Remove gossip model reference if switching to uniform mode
                    self.model.behavior_model.gossip_model = None
                    self._update_status("Switched to uniform information diffusion")
                    
                    # Disable gossip tab
                    self.notebook.tab(4, state="disabled")
        
        # Update the info mode indicator
        self.info_mode_indicator_var.set(f"Info Mode: {self.params.information_diffusion_mode.capitalize()}")
    ...
    def _update_plots(self):
        ...
        elif current_tab == 4:  # Gossip tab
            self._update_gossip_viz()
        elif current_tab == 5:  # Data tab
